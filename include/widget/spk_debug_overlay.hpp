...
namespace spk
{
	...
	private:
		...
		{
			spk::Font::Size fontSize = spk::Font::Size(
				geometry().size.y,
				_outlineSize
			);
	...
	public:
			...
			spk::Widget(p_name, p_parent)
		{
			for (size_t i = 0; i < Column; i++)
			{
				for (size_t j = 0; j < Row; j++)
				{
					_labels[i][j] = std::make_unique<spk::TextLabel>(p_name + L" - Label [" + std::to_wstring(i) + L"][" + std::to_wstring(j) + L"]", this);
		...
		void setText(const size_t& p_col, const size_t& p_row, const std::wstring& p_text)
		{
			...
			{
				throw std::invalid_argument("Can't set debug overlay [" + std::to_string(Column) + " / " + std::to_string(Row) + "] text at coordinate [" + std::to_string(p_col) + " / " + std::to_string(p_row) + "]");
			}
