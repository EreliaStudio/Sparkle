		...
		public:
			...
			bool containSampler(const std::wstring& p_name) const
			{
				return (_originator->containSampler(p_name) || 
						...
						std::holds_alternative<OpenGL::SamplerObject>(_attributes.at(p_name)) == true));
			}
			...
			{
				return (_originator->containUBO(p_name) || 
						(_attributes.contains(p_name) == true &&
	...
	private:
		...
		void addSamplerConstant(const std::wstring &p_name, OpenGL::SamplerObject &&p_object)
		{
			...
			else 
			{
				...
				{
					throw std::runtime_error("Object with name '" + spk::StringUtils::wstringToString(p_name) + "' is already set but is not a sampler object.");
				}
		...
		void addSamplerAttribute(const std::wstring &p_name, OpenGL::SamplerObject &&p_object)
		{
			if (_attributes.contains(p_name) == true)
			{
				...
				{
					throw std::runtime_error("Attribute with name '" + spk::StringUtils::wstringToString(p_name) + "' is already set but is not a sampler object.");
				}
		...
		void addUBOConstant(const std::wstring &p_name, OpenGL::UniformBufferObject &&p_object)
		{
			...
			else 
			{
				...
				{
					throw std::runtime_error("Object with name '" + spk::StringUtils::wstringToString(p_name) + "' is already set but is not an UBO.");
				}
				...
				{
					throw std::runtime_error("UBO size mismatch for '" + spk::StringUtils::wstringToString(p_name) + "'. Expected: " + std::to_string(ubo->size()) + ", got: " + std::to_string(p_object.size()));
				}

				if (ubo->bindingPoint() != p_object.bindingPoint())
				{
					throw std::runtime_error("UBO binding point mismatch for '" + spk::StringUtils::wstringToString(p_name) + "'. Expected: " + std::to_string(ubo->bindingPoint()) + ", got: " + std::to_string(p_object.bindingPoint()));
				}
			}
		...
		void addUBOAttribute(const std::wstring &p_name, OpenGL::UniformBufferObject &&p_object)
		{
			if (_attributes.contains(p_name) == true)
			{
				...
				{
					throw std::runtime_error("Attribute with name '" + spk::StringUtils::wstringToString(p_name) + "' is already set but is not an UBO object.");
				}
		...
		void addSSBOConstant(const std::wstring &p_name, OpenGL::ShaderStorageBufferObject &&p_object)
		{
			...
			else 
			{
				...
				{
					throw std::runtime_error("Object with name '" + spk::StringUtils::wstringToString(p_name) + "' is already set but is not an SSBO.");
				}
				...
				{
					throw std::runtime_error("SSBO fixed data size mismatch for '" + spk::StringUtils::wstringToString(p_name) + "'. Expected: " + std::to_string(ssbo->fixedData().size()) + ", got: " + std::to_string(p_object.fixedData().size()));
				}

				if (ssbo->dynamicArray().elementSize() != p_object.dynamicArray().elementSize())
				{
					throw std::runtime_error("SSBO dynamic array element size mismatch for '" + spk::StringUtils::wstringToString(p_name) + "'. Expected: " + std::to_string(ssbo->dynamicArray().elementSize()) + ", got: " + std::to_string(p_object.dynamicArray().elementSize()));
				}

				if (ssbo->bindingPoint() != p_object.bindingPoint())
				{
					throw std::runtime_error("SSBO binding point mismatch for '" + spk::StringUtils::wstringToString(p_name) + "'. Expected: " + std::to_string(ssbo->bindingPoint()) + ", got: " + std::to_string(p_object.bindingPoint()));
				}
			}
		...
		void addSSBOAttribute(const std::wstring &p_name, OpenGL::ShaderStorageBufferObject &&p_object)
		{
			if (_attributes.contains(p_name) == true)
			{
				...
				{
					throw std::runtime_error("Attribute with name '" + spk::StringUtils::wstringToString(p_name) + "' is already set but is not an SSBO.");
				}
