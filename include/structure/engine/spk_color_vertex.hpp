...
namespace std
{
	...
	struct hash<spk::ColorVertex>
	{
		size_t operator()(const spk::ColorVertex &p_vertex) const
		{
			...
			size_t h2 = std::hash<float>{}(p_vertex.color.r);
			size_t h3 = std::hash<float>{}(p_vertex.color.g);
			size_t h4 = std::hash<float>{}(p_vertex.color.b);
			size_t h5 = std::hash<float>{}(p_vertex.color.a);
			size_t seed = h1;
