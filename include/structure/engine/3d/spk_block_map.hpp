...
#include "structure/engine/3d/spk_collision_mesh_renderer.hpp"
#include "structure/engine/3d/spk_entity.hpp"
#include "structure/engine/3d/spk_obj_mesh.hpp"
#include "structure/engine/3d/spk_obj_mesh_renderer.hpp"
...
namespace spk
{
	...
	class BlockMap : public spk::GenericEntity
	{
	public:
		class Chunk : public spk::GenericEntity
		{
		public:
			...
			public:
				...
				void onPaintEvent(spk::PaintEvent &p_event) override
				{
					if (_isBaked == false)
					{
						...
						{
							_renderer->seGenericMesh(mesh());
							_collisionRenderer->seGenericMesh(collisionMesh());
		...
		public:
			Chunk(const std::wstring &p_name, spk::SafePointer<BlockMap> p_parent) :
				spk::GenericEntity(p_name, p_parent)
			{
	...
	public:
		BlockMap(const std::wstring &p_name, spk::SafePointer<spk::GenericEntity> p_parent) :
			spk::GenericEntity(p_name, p_parent)
		{
		}
