...
namespace spk
{
	...
	private:
		...
		{
			if (p_shape.vertices.size() < 3)
			{
				return;
			}

			unsigned int ia = _getOrInsertVertex(p_shape.vertices[0], p_vertexMap);
			for (std::size_t i = 1; i + 1 < p_shape.vertices.size(); ++i)
			{
				unsigned int ib = _getOrInsertVertex(p_shape.vertices[i], p_vertexMap);
				unsigned int ic = _getOrInsertVertex(p_shape.vertices[i + 1], p_vertexMap);
