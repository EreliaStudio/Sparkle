#include "structure/engine/2d/spk_collision_mesh_2d_renderer.hpp"
...
outputColor = vec4(color, 1.0);
	...
	void CollisionMesh2DRenderer::Painter::setTransform(const spk::Transform2D &p_transform)
	{
	...
	void CollisionMesh2DRenderer::seGenericMesh(const spk::SafePointer<const spk::CollisionMesh2D> &p_mesh)
	{
		_mesh = p_mesh;
		_painter.clear();
		if (_mesh != nullptr)
	...
	{
		_onOwnerTransformEditionContract = owner().upCast<spk::Entity2D>()->transform().addOnEditionCallback([this]() { _painter.setTransform(owner().upCast<spk::Entity2D>()->transform()); });
