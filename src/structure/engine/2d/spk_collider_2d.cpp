...
namespace spk
{
	...
	{
		_onOwnerOnTransformEditionContract = owner().upCast<spk::Entity2D>()->transform().addOnEditionCallback(
			[&]()
			{
	...
	namespace
	{
		std::vector<spk::Polygon2D> collectPolygons2D(
			const spk::SafePointer<const spk::CollisionMesh2D> &p_collisionMesh, const spk::Transform2D &p_transform)
		{
			...
			const spk::Vector2 offset = p_transform.position();
	...
	void Collider2D::_updateCollisionCache()
	{
		...
		{
			_boundingBox = _collisionMesh->boundingBox().place(owner().upCast<spk::Entity2D>()->transform().position());
			_polygons = collectPolygons2D(_collisionMesh, owner().upCast<spk::Entity2D>()->transform());
		}
		else
		{
