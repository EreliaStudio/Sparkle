#include "structure/engine/2d/spk_ray_cast_2d.hpp"
...
namespace
{
	...
	void addHit(
		...
		float p_t,
		const spk::SafePointer<spk::Entity2D> &p_owner,
	...
	void processColliderBody(
		const spk::SafePointer<spk::Collider2D> &p_collider,
		...
		std::vector<spk::RayCast2D::Hit> &p_hits)
	{
		...
		spk::SafePointer<spk::Entity2D> owner = p_collider->owner();
		...
		spk::Vector2 offset = owner->transform().position();
...
namespace spk
{
	namespace
	{
			...
			spk::BinaryOperator p_binaryOperator)
		{
			...
			const std::vector<spk::SafePointer<spk::Collider2D>> &colliders = spk::Collider2D::getColliders();
		...
		spk::BinaryOperator p_binaryOperator)
	{
		...
		spk::Vector2 eye = p_cameraEntity->transform().position();
		...
		const spk::SafePointer<spk::Entity2D> &p_cameraEntity, const spk::Vector2 &p_screenPosition, float p_layer)
	{
		...
		auto camComp = p_cameraEntity->getComponent<spk::CameraComponent2D>();
