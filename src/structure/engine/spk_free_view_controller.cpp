...
namespace spk
{
	...
	FreeViewController::FreeViewController(const std::wstring &p_name) : spk::Component(p_name)
	{
	...
	void FreeViewController::onUpdateEvent(spk::UpdateEvent &p_event)
	{
		...
		if (isMotionRequested == true || isRotationRequested == true)
		{
			...
			{
				auto delta = _motionRequested.normalize() * static_cast<float>(p_event.deltaTime.seconds) * _config.moveSpeed;
