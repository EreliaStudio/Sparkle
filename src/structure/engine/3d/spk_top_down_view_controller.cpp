#include "structure/engine/3d/spk_top_down_view_controller.hpp"
...
namespace spk
{
		...
		spk::Component(p_name)
	{
		_actions.push_back(
			std::make_unique<spk::KeyboardAction>(
				...
				{
					spk::Vector3 forward = owner().upCast<spk::Entity>()->transform().forward() * spk::Vector3(1, 0, 1);

					if (forward == spk::Vector3())
					{
						forward = owner().upCast<spk::Entity>()->transform().up() * spk::Vector3(1, 0, 1);
					}
		...
		_actions.push_back(
			std::make_unique<spk::KeyboardAction>(
				...
				[&](const spk::SafePointer<const spk::Keyboard> &p_keyboard)
				{
					spk::Vector3 right = owner().upCast<spk::Entity>()->transform().right();
					right.y = 0;
		...
		_focusPoint = owner().upCast<spk::Entity>()->transform().position() + owner().upCast<spk::Entity>()->transform().forward();
	}
	...
	{
		_focusPoint = p_focus;
		owner().upCast<spk::Entity>()->transform().lookAt(_focusPoint);
	}
	...
	void TopDownViewController::onUpdateEvent(spk::UpdateEvent &p_event)
	{
		...
		if (isMotionRequested == true || isRotationRequested == true)
		{
			if (_motionRequested != spk::Vector3())
			{
				...
				owner().upCast<spk::Entity>()->transform().move(delta);
				_focusPoint += delta;
			}
			...
			if (_rotationRequested != 0.0f)
			{
				...
				spk::Vector3 offset = owner().upCast<spk::Entity>()->transform().position() - _focusPoint;
				spk::Quaternion rot = spk::Quaternion::fromAxisAngle({0, 1, 0}, angle);
				offset = rot * offset;
				owner().upCast<spk::Entity>()->transform().place(_focusPoint + offset);
				owner().upCast<spk::Entity>()->transform().lookAt(_focusPoint);
			}
