...
namespace spk
{
	void CameraComponent::_updateUBO()
	{
		...
		const spk::Transform &transform = owner().upCast<spk::Entity>()->transform();
	...
	{
		_transformContract = owner().upCast<spk::Entity>()->transform().addOnEditionCallback([this]() { _updateUBO(); });
		_updateUBO();
	}
