#include "structure/engine/3d/spk_free_view_controller.hpp"
...
namespace spk
{
		...
		spk::Component(p_name)
	{
		_actions.push_back(
			std::make_unique<spk::KeyboardAction>(
				...
				spk::InputState::Down,
				10,
				[&](const spk::SafePointer<const spk::Keyboard> &p_keyboard) { _motionRequested += owner().upCast<spk::Entity>()->transform().forward(); }));
		...
		_actions.push_back(
			std::make_unique<spk::KeyboardAction>(
				...
				spk::InputState::Down,
				10,
				[&](const spk::SafePointer<const spk::Keyboard> &p_keyboard) { _motionRequested -= owner().upCast<spk::Entity>()->transform().right(); }));
	...
	void FreeViewController::onUpdateEvent(spk::UpdateEvent &p_event)
	{
		...
		if (isMotionRequested == true || isRotationRequested == true)
		{
			...
			{
				owner().upCast<spk::Entity>()->transform().rotateAroundAxis({0, 1, 0}, _rotationRequested.x);
				owner().upCast<spk::Entity>()->transform().rotateAroundAxis(owner().upCast<spk::Entity>()->transform().right(), _rotationRequested.y);
