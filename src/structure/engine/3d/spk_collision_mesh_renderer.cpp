...
void main()
{
	...
	void CollisionMeshRenderer::Painter::prepare(const spk::CollisionMesh &p_mesh)
	{
		...
		for (const auto &unit : p_mesh.units())
		{
			...
			offset += static_cast<unsigned int>(pts.size());
		}
	}
	...
	void CollisionMeshRenderer::seGenericMesh(const spk::SafePointer<const spk::CollisionMesh> &p_mesh)
	{
		_mesh = p_mesh;
	...
	{
		_onOwnerTransformEditionContract = owner().upCast<spk::Entity>()->transform().addOnEditionCallback([this]() { _painter.setTransform(owner().upCast<spk::Entity>()->transform()); });
