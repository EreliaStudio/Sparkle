...
namespace spk
{
	...
	ActivableObject::ActivationContract ActivableObject::addActivationCallback(const Job &p_callback)
	{
		return (std::move(addCallback(true, p_callback)));
	}

	ActivableObject::DeactivationContract ActivableObject::addDeactivationCallback(const Job &p_callback)
	{
