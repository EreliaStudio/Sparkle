...
{
	std::wstring Shader::_extractSection(const std::wstring &p_src, const std::wstring &p_sectionName)
	{
	...
	spk::OpenGL::LayoutBufferObject::Attribute::Type Shader::_toAttrType(const std::wstring &p_input)
	{
	...
	std::vector<Shader::Layout> Shader::_parseLayoutLines(const std::wstring &p_section)
	{
		std::vector<Shader::Layout> list;
	...
	void Shader::_parseExceptionSection(const std::wstring &p_section)
	{
		if (p_section.empty())
		{
			return;
	...
	std::vector<spk::JSON::Object> Shader::_splitIntoJsonObjects(const std::wstring &p_section)
	{
	...
	void Shader::_parseTextureSection(const std::wstring &p_section)
	{
		std::wistringstream ss(p_section);
		std::wstring line;
		while (std::getline(ss, line))
	...
	Shader::Shader(const std::wstring &p_source)
	{
	...
	Shader Shader::fromSource(const std::wstring &p_src)
	{
		return Shader(p_src);
	}
	Shader Shader::fromFile(const std::wstring &p_path)
	{
		return fromSource(spk::FileUtils::readFileAsWString(p_path));
	}
	
	std::wostream& operator << (std::wostream& p_os, const Shader& p_shader)
	{
	...
	std::ostream& operator << (std::ostream& p_os, const Shader& p_shader)
	{
