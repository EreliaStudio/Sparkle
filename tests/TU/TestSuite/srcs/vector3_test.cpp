...
namespace
{
	...
	TEST(Vector3Test, ToJSONSerializesComponentsAsArray)
	{
		const spk::Vector3 value{1.5f, -2.25f, -3.125f};
	...
	TEST(Vector3Test, FromJSONDeserializesComponentsFromArray)
	{
		...
		const spk::Vector3 value = spk::Vector3::fromJSON(json);

		EXPECT_EQ(value, spk::Vector3(1.5f, -2.25f, -3.125f));
	}

	TEST(Vector3Test, ConstructorFromJSONDeserializesComponentsFromArray)
	{
		spk::JSON::Value json = spk::JSON::Value::array();
		json.pushBack(3.5f);
		json.pushBack(-4.25f);
	...
	{
		const spk::Vector3 original{3.25f, -7.5f, -5.125f};

		const spk::Vector3 result =
			spk::Vector3::fromJSON(original.toJSON());
