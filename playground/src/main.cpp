...
protected:
	...
	{
		auto transform = [&](spk::Vertex &p_v) { p_v.uv = (p_v.uv * p_sprite.size) + p_sprite.anchor; };
		std::visit(
			[&](auto &p_face)
			{
				transform(p_face.a);
...
public:
			  ...
			  const Orientation &p_orientation) const
	{
		...
		for (size_t i = 0; i < 6; ++i)
		{
			...
			{
				spk::Polygon ourWorldFootprint = ourFace.footprint;
				for (auto &pt : ourWorldFootprint.points)
				{
					pt = pt + p_position;
				}

				spk::Polygon neighWorldFootprint = currentNeighFace->footprint;
				for (auto &pt : neighWorldFootprint.points)
				{
					pt = pt + p_position + Block::neightbourCoordinates[i];
				}

				if (neighWorldFootprint.contains(ourWorldFootprint) == true)
				{
					visible = false;
				}
			}
