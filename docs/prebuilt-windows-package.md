...
`.github/workflows/prebuilt-windows-package.yml` produces Sparkle's reusable Windows package. Its version is read from the root `vcpkg.json` manifest unless explicitly overridden. It configures a clean Release build with clang-cl, static CRT, static Sparkle, and `SPARKLE_BUILD_TEST_LIBRARY=ON`, then installs through CMake into a clean prefix.
...
These prebuilt releases are an optimization for controlled downstream CI such as Playground. They are intentionally separate from the public `erelia-sparkle` vcpkg port, which is expected to download a tagged Sparkle source release and build it through vcpkg.
