...
namespace spk::Network
{
	...
	private:
        ...
        bool _isRunning;
		std::thread *_acceptorPersistantWorker; /**< Persistent worker for accepting new connection */
		std::thread *_messageReceptionWorker; /**< Persistent worker for receiving new message */

		Acceptor _Acceptor; /**< The server socket acceptor */
        spk::Network::Socket::FileDescriptor _maxFDs = Socket::SocketError;
        fd_set _readingFDs;

		spk::ThreadSafeDeque<std::pair<EmiterID, spk::Network::Message>> _messagesToTreat; /**< Queue for storing incoming messages */
		...
		std::function<void(const EmiterID&, const spk::Network::Message&)> _onUnknownMessageTypeCallback = [&](const EmiterID& p_id, const spk::Network::Message& p_msg){
			std::cout << "Callback not defined for message id [" << std::to_string(p_msg.header().type()) << "]" << std::endl;
		};
