...
namespace spk::Network
{
	...
	{
	public:
		std::thread *_socketContextWorker; /**< A worker object that handles tasks related to the socket context */

		spk::Network::Socket _socket; /**< The socket object used for network communication */
		spk::ThreadSafeDeque<spk::Network::Message> _messagesToTreat; /**< A thread-safe queue that stores messages to be processed */
		...
		std::function<void(const spk::Network::Message&)> _onUnknowMessageReception = [&](const spk::Network::Message& p_msg){
			std::cout << "Unknow message ID [" << p_msg.header().type() << "] received" << std::endl;
	...
	public:
		...
		void setOnMessageReceptionCallback(const TMessageType& p_messageType, std::function<void(const spk::Network::Message&)> p_funct)
        {
            ...
            {
                spk::throwException("Callback already define for message type [" + std::to_string(messageType) + "]");
            }
