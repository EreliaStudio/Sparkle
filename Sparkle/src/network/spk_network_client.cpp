...
namespace spk::Network
{
				...
				FD_ISSET(_socket.fileDescriptor(), &socketToRead))
			{
				...
				switch (readStatus)
				{
					case Socket::ReadResult::Closed:
						std::cout << "Require closing socket" << std::endl;
						_socket.close();
						break;
	...
	Client::Client() : _socketContextWorker(new std::thread([&]()
			{
				_receiveMessage();
			}))
	{
	...
	Client::~Client()
	{
		if (_socketContextWorker->joinable())
			_socketContextWorker->join();
		delete _socketContextWorker;
	}

	void Client::connect(const std::string &p_serverAddress, const size_t &p_serverPort)
	{
	...
	void Client::send(const spk::Network::Message &p_msg)
	{
		if (_socket.isConnected() == false)
			spk::throwException("Can't send a message throught a disconnected socket");
		_socket.send(p_msg);
	}
}