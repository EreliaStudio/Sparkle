...
namespace spk::Network
{
	...
	long long Server::_findValidID()
	{
		...
		void Server::_receiveNewMessage()
		{
			if (_maxFDs == -1)
				return ;
			...
			{
				spk::throwException("Error while receiving message inside server process [" + std::to_string(Socket::getLastError()) + "]");
			}
			else if (activity == 0)
			{
				return ;
	...
	Server::Server() :
		_acceptorPersistantWorker(nullptr),
		_messageReceptionWorker(nullptr)
	{
		FD_ZERO(&_readingFDs);
	}
	...
	void Server::start(const size_t& p_serverPort)
	{
		...
		_isRunning = true;
		_acceptorPersistantWorker = new std::thread([&]()
			{
		...
		_messageReceptionWorker = new std::thread([&]()
			{
				while (_isRunning == true)
				{
					_receiveNewMessage();	
	...
	void Server::stop()
	{
		...
		if (_acceptorPersistantWorker->joinable())
			_acceptorPersistantWorker->join();
		if (_messageReceptionWorker->joinable())
			_messageReceptionWorker->join();

		delete _acceptorPersistantWorker;
		delete _messageReceptionWorker;
	}
	...
	void Server::sendTo(const Server::EmiterID& p_emiterID, const spk::Network::Message& p_msg)
	{
		if (_clients.contains(p_emiterID) == false)
			spk::throwException("Emiter link to no client");
