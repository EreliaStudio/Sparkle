...
namespace spk::widget {
  ...
  class LayoutBuilder : public IWidget {
    public:
      ...
      LayoutBuilder(const Builder& p_builder) : _builder(p_builder) {}
  ...
  class SizedBox : public SingleChildWidget {
    public:
      SizedBox(const Vector2& p_size) : _size(p_size) {}
    ...
    private:
      Vector2 _onLayout(const BoxConstraints& p_constraints) override {
        ...
        Vector2 max = Vector2::min(p_contraints.max, _size);
        Vector2 min = Vector2::max(p_contraints.min, _size);

        Vector2 childSize = child->_onLayout(BoxConstraints(min, max));
        child->setGeometry(childSize);
  ...
  class FractionallySizedBox : public SingleChildWidget {
    public:
      FractionallySizedBox(float p_horizontal, float p_vertical) :
  ...
  class Padding : public SingleChildWidget {
    public:
      ...
      Padding(const Config& p_config) : _config(p_config) {}
      static Padding symmetric(float p_horizontal, float p_vertical) { return Padding({p_horizontal, p_horizontal, p_vertical, p_vertical}); }
      static Padding all(float p_value) { return Padding({p_value, p_value, p_value, p_value}); }

    private:
      Vector2 _onLayout(const BoxConstraints& p_constraints) {
        IWidget* child = child();
        if (nullptr != child) {
          Vector2 padded = Vector2{left + right, top + bottom};
          Vector2 max = p_constraints.max - padded;
          Vector2 min = p_constraints.min - padded;
          Vector2 childSize = child->_onLayout({min, max});

          Vector2 anchor{left, top};
  ...
  class Expanded : public SingleChildWidget {
    public:
      ...
      */
      Expanded(double p_flex = 1.0f) : _flex(p_flex);

      double flex() const { return _flex; }
  ...
  class Column : public IWidget {
    ...
    Column(const Config& p_config = Config()) : _config(p_config) {}
    ...
    private:
      Vector2 _onLayout(const BoxConstraints& p_constraints) override {
        ...
        for (int i = 0; i < len; i++) {
          ...
          Expanded* asExpanded = std::dynamic_cast<Expanded*>(child)
        ...
        size_t availableHeight = p_constraints.max.y - height;
        ...
        switch (_config.mainAxisAlignment) {
          ...
          case MainAxisAlignment::center:
            baseY = (p_constraints.y - height) / 2;
            break;
          case MainAxisAlignment::end:
            baseY = p_constraints.y - height;
            break;
        }

        float currentY = p_baseY;
        for (int i = 0; i < children().size(); i++) {
          const Vector2& size = p_sizes[i];
          IWidget* child = children()[i];
  ...
  class Row : public IWidget {
    ...
    private:
      Vector2 _onLayout(const BoxConstraints& p_constraints) override {
        ...
        size_t availableWidth = p_constraints.max.x - width;
        if (availableWidth < 0) {
          availableWidth = 0;
        }
        ...
        for (int i = 0; i < len; i++) {
          ...
          if (nullptr != asExpended) {
            ...
            Vector2 max{childWidth, p_constraints.y};
        ...
        switch (_config.mainAxisAlignment) {
          case MainAxisAlignment::start:
            ...
            break;
          case MainAxisAlignment::center:
            baseX = (p_constraints.x - width) / 2;
            break;
          case MainAxisAlignment::end:
            baseX = p_constraints.x - width;
            break;
        }
