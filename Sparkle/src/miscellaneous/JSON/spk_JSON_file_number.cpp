...
namespace spk
{
	namespace JSON
	{
		 ...
		 * @param p_decimalPos Location of the decimal point. (std::string::npos if there is no decimal point)
		 * @param p_exponentPos Location of the exponent. (std::string::npos if there is no exponent)
		...
		static bool _isNumberMalformatted(bool p_isNegative, const size_t& p_decimalPos,
			const size_t& p_exponentPos, const std::string& p_unitSubString)
		{
			return (p_exponentPos == p_unitSubString.size() - 1 ||
				...
				(p_decimalPos != std::string::npos && ::isdigit(p_unitSubString[p_decimalPos + 1]) == false) ||
				(p_decimalPos != std::string::npos && p_exponentPos != std::string::npos && p_decimalPos > p_exponentPos) ||
				(p_unitSubString[p_isNegative] == L'0' && p_unitSubString.size() > static_cast<size_t>(p_isNegative) + 1u &&
					std::string(".eE").find(p_unitSubString[p_isNegative + 1]) == std::string::npos) ||
		 ...
		 */
		static long _extractExponent(const std::string& p_exponentSubstring)
		{
			bool isExponentSigned(p_exponentSubstring[0] == L'-' || p_exponentSubstring[0] == L'+');
			long result(0);

			if (p_exponentSubstring.find_first_not_of("0123456789", isExponentSigned) != std::string::npos)
				throwException("Invalid numbers JSON exponent value: " + p_exponentSubstring);
			try
			{
			...
			{
				throwException("Invalid numbers JSON value: " + p_exponentSubstring + " too big (number overflow)");
			}
			return (result);
		}
		 ...
		 *
		 * @param p_decimalPos The position of the decimal point. (std::string::npos if there is no decimal point)
		...
		{
			if (p_decimalPos == std::string::npos && p_exponentPos == false)
				return false;
			return (p_decimalPos != std::string::npos || (p_hasExponent == true &&
		 ...
		 */
		static long _safePowerOfTen(const long& p_number, const long& p_exponent, const std::string& p_unitSubString)
		{
			long result(0);
			errno = 0;
			...
			if (errno == EDOM || errno == ERANGE || std::fetestexcept(FE_ALL_EXCEPT ^ FE_INEXACT) != 0)
				throwException("Invalid numbers JSON value: " + p_unitSubString + " too big (power overflow)");
		...
		void _loadUnitNumbers(spk::JSON::Object& p_objectToFill, const std::string& p_unitSubString)
		{
			...
			size_t exponentPos(p_unitSubString.find_first_of("eE"));
			long exponent(0);

			size_t decimalPos(p_unitSubString.find_last_of("."));

			if (_isNumberMalformatted(isNegative, decimalPos, exponentPos, p_unitSubString) == true)
				throwException("Malformatted JSON number: " + p_unitSubString);

			exponentPos = (exponentPos == std::string::npos) ? p_unitSubString.size() : exponentPos;

			if (p_unitSubString.substr(isNegative, exponentPos - isNegative).find_first_not_of(".0123456789") != std::string::npos)
				throwException("JSON number value is not Numerical: " + p_unitSubString);
			...
			{
				throwException("Invalid numbers JSON value: " + p_unitSubString + " too big (number overflow)");
			}
