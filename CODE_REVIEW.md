...
3. **Patterns and traits:** `ContractProvider` handles same-thread reentrancy and now serializes cross-thread subscription, resignation, invalidation, validity checks, and dispatch through its shared state. Hierarchy, context, registry, activation, and naming are currently composed through many inheritance bases; explicit owned collaborators will scale better than additional traits.
