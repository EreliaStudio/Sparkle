...
namespace spk::WinAPI
{
	...
	void Frame::Class::_register()
	{
		...
		description.lpfnWndProc = &Frame::_procedure;
	...
	int Frame::_wideSize(std::string_view value)
	{
	...
	void Frame::_writeWide(std::string_view value, std::wstring &result)
	{
	...
	SIZE Frame::_outerSize(const CreationInfo &info)
	{
	...
	Frame *Frame::_instance(HWND handle) noexcept
	{
		return reinterpret_cast<Frame *>(::GetWindowLongPtrW(handle, GWLP_USERDATA));
	}

	Frame *Frame::_bind(HWND handle, LPARAM parameter) noexcept
	{
	...
	LRESULT Frame::_process(HWND handle, UINT message, WPARAM wParam, LPARAM lParam) noexcept
	{
		if (message == WM_CLOSE)
		{
	...
	LRESULT Frame::_processDestruction(HWND handle, UINT message, WPARAM wParam, LPARAM lParam) noexcept
	{
	...
	LRESULT CALLBACK Frame::_procedure(HWND handle, UINT message, WPARAM wParam, LPARAM lParam) noexcept
	{
	...
	HWND Frame::handle() const noexcept
	{
		return _handle;
	}
